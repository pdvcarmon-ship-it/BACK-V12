...
"""
    ...
    if geojson:
        try:
            geojson_data = json.loads(geojson)
            bbox_floats = list(map(float, bbox.split(",")))
            mask = geojson_to_mask(geojson_data, img_gray.width, img_gray.height, bbox_floats)
            mask_arr = np.array(mask) > 128
        except Exception as e:
    ...
    for zona in ZONAS_NDVI:
        ...
        superficie_m2 = pixel_count * PIXEL_AREA_M2
        ...
        zonas_stats.append({
            ...
            "pixeles": pixel_count,
            "superficie_m2": round(superficie_m2, 1),
